...
class FetchBookmarkedNotificationsUseCaseTest {

    private val repository = mockk<NotificationRepository>()
    private val useCase = FetchBookmarkedNotificationsUseCase(repository)

    @Test
    fun `invoke returns repository flow`() {
        val flow = flowOf(PagingData.empty<NotificationModel>())
        every { repository.fetchBookmarkedNotifications() } returns flow
